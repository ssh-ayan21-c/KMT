export default function NotificationsMenu() {
    ...
    return (
         <>
              ...
              {showModal && (
                   <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)', background: 'rgba(0,0,0,0.8)' }} onClick={() => setShowModal(false)}>
                        <div style={{ width: '500px', maxHeight: '80vh', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }} onClick={e => e.stopPropagation()}>
                             <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255, 90, 9, 0.05)' }}>
                                  <div>
                             ...
                             <div style={{ padding: '1rem', maxHeight: '500px', overflowY: 'auto' }}>
                                  {notifications.length === 0 ? (
                                       ...
                                       <div key={notif._id} style={{ padding: '1.25rem', borderBottom: '1px solid var(--border-color)', background: notif.read ? 'transparent' : 'rgba(255, 90, 9, 0.03)', cursor: 'pointer', display: 'flex', gap: '1.25rem', transition: 'var(--transition)', borderRadius: '12px', marginBottom: '0.5rem' }} onClick={() => { if(!notif.read) markAsRead(notif._id) }}>
                                            ...
                                            <div style={{ width: '40px', height: '40px', borderRadius: '12px', background: notif.read ? 'var(--bg-primary)' : 'rgba(255, 90, 9, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', shrink: 0 }}>
                             ...
                             <div style={{ padding: '1rem', textAlign: 'center', background: 'rgba(0,0,0,0.2)', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
